<template>
  <div>
    ...
    <div style="height: 10vh">
      <div id="myBarChartReview" style="height: inherit"></div>
    </div>
  </div>
</template>
  ...
  methods: {
    async fetchData() {
      ...
      responseData2.forEach((attr) => {
        this.AirbnbData.number_of_reviews.push(attr.number_of_reviews);
      });
    ...
    drawScatterPlot() {
      let array = this.AirbnbData.number_of_reviews;
      let interval = 10;
      let intervals = [];
      ...
      Plotly.newPlot("myBarChartReview", data, layout, config);
    },
  },
};
</script>
